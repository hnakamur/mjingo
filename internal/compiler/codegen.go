...
type codeGenerator struct {
	...
	currentLine      uint32
	spanStack        stack.Stack[common.Span]
...
func (g *codeGenerator) compileMacroExpression(macroDecl macroStmt) {
	...
	for _, arg := range macroDecl.args {
		if varExp, ok := arg.(varExpr); ok {
			ids = append(ids, value.ValueFromString(varExp.id))
		} else {
			panic("unreachable")
		}
	}
	g.add(LoadConstInstruction{Val: value.ValueFromSlice(ids)})
	flags := uint8(0)
	if callerReference {
		flags |= MacroCaller
	}
...
func (g *codeGenerator) compileExpr(exp expression) {
	...
	case sliceExpr:
		...
		if exp.start.IsSome() {
			...
			g.add(LoadConstInstruction{Val: value.ValueFromI64(int64(0))})
		}
		if exp.stop.IsSome() {
		...
		if exp.step.IsSome() {
			...
			g.add(LoadConstInstruction{Val: value.ValueFromI64(int64(1))})
		}
	...
	case kwargsExpr:
		...
		if optVal.IsSome() {
			...
			for _, pair := range exp.pairs {
				g.add(LoadConstInstruction{Val: value.ValueFromString(pair.key)})
...
func (g *codeGenerator) compileCall(c call, spn common.Span, caller option.Option[macroStmt]) {
	g.pushSpan(spn)
...
func (g *codeGenerator) compileCallArgsWithCaller(args []expression, caller macroStmt) uint {
	...
	for _, arg := range args {
		if m, ok := arg.(kwargsExpr); ok {
			...
			g.add(LoadConstInstruction{Val: value.ValueFromString("caller")})
...
func (g *codeGenerator) setLineFromSpan(spn common.Span) {
	g.setLine(spn.StartLine)
}

func (g *codeGenerator) pushSpan(spn common.Span) {
...
func (g *codeGenerator) addWithSpan(instr Instruction, spn common.Span) uint {
	return g.instructions.addWithSpan(instr, spn)
}

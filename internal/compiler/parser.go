...
func (s *tokenStream) next() (token, *common.Span, error) {
...
func (s *tokenStream) current() (token, *common.Span, error) {
	return s.curToken, s.curSpan, s.curErr
}

func (s *tokenStream) expandSpan(span common.Span) common.Span {
	return common.Span{
...
func (s *tokenStream) currentSpan() common.Span {
	if s.curSpan != nil {
		return *s.curSpan
	}
	return s.lastSpan
...
func (p *parser) parseArgs() ([]expression, error) {
	args := []expression{}
	firstSpan := option.None[common.Span]()
	kwargs := []kwargExpr{}
...
func (p *parser) parsePostfix(exp expression, spn common.Span) (expression, error) {
loop:
	for {
...
func (p *parser) parsePrimaryImpl() (expression, error) {
	...
	case identToken:
		switch tkn.ident {
		case "true", "True":
			return makeConst(value.ValueFromBool(true), *spn), nil
		case "false", "False":
			return makeConst(value.ValueFromBool(false), *spn), nil
		case "none", "None":
	...
	case stringToken:
		return makeConst(value.ValueFromString(tkn.s), *spn), nil
	case intToken:
		return makeConst(value.ValueFromI64(tkn.n), *spn), nil
	case floatToken:
		return makeConst(value.ValueFromF64(tkn.f), *spn), nil
	case parenOpenToken:
...
func (p *parser) parseListExpr(spn common.Span) (expression, error) {
...
func (p *parser) parseMapExpr(spn common.Span) (expression, error) {
...
func (p *parser) parseTupleOrExpression(spn common.Span) (expression, error) {
...
func (p *parser) expectToken(f func(tkn token) bool, expected string) (token, common.Span, error) {
	tkn, spn, err := p.stream.next()
	if err != nil {
		return nil, common.Span{}, err
	}
	if tkn == nil {
		return nil, common.Span{}, unexpectedEOF(expected)
	}
	if f(tkn) {
		return tkn, *spn, nil
	}
	return nil, common.Span{}, unexpected(tkn, expected)
}
...
func unexpected(unexpected any, expected string) error {
	return common.NewError(common.SyntaxError,
...
func makeConst(v value.Value, spn common.Span) expression {
	return constExpr{val: v, span: spn}
}

func syntaxError(msg string) error {
	return common.NewError(common.SyntaxError, msg)
}

...
func (e kwargsExpr) asConst() option.Option[value.Value] {
	...
	rv := value.ValueMapWithCapacity(uint(len(e.pairs)))
	for _, pair := range e.pairs {
		if v, ok := pair.arg.(constExpr); ok {
			rv.Set(value.KeyRefFromValue(value.ValueFromString(pair.key)), v.val.Clone())
		}
	}
	return option.Some(value.ValueFromKwargs(value.NewKwargs(*rv)))
}
...
func (l listExpr) asConst() option.Option[value.Value] {
	...
	return option.Some(value.ValueFromSlice(seq))
}
...
func (m mapExpr) asConst() option.Option[value.Value] {
	...
	rv := value.ValueMapWithCapacity(uint(len(m.keys)))
	for i, key := range m.keys {
		val := m.values[i]
		if key.typ() == exprTypeConst && val.typ() == exprTypeConst {
			keyRf := value.KeyRefFromValue(key.(constExpr).val)
			rv.Set(keyRf, val.(constExpr).val)
		}
	}
	return option.Some(value.ValueFromIndexMap(rv))
}
